...
from schemas.todo import Usuarios,Frutas
...
@app.get ("/users/{id}")
async def ver_usuairo_por_id(id:int):
    # Se busca el usuario por el id proporcionado
    user_query = session.query(Usuarios).filter(Usuarios.id == id)
...
@app.post("/users/")
async def crear_usuario(nombre:str, presupuesto:int): # los parametros deben ser los mismos de la tabla de la class "Todo"
    user = Usuarios(nombre=nombre, presupuesto=presupuesto)
    session.add(user)    # se le dice a la session 'db' que agregue 'todo 'con todas sus instancias'
    session.commit()     # commit es para que se ejecute la transaccion
    return {"usuario agregado" : user.nombre}


" end point .put para actualizar datos de 'usuarios' "
@app.put("/users/{id}")
async def actualizar_usuario(id: int, nombre: str, presupuesto: int):
    # Se busca el usuario por el id proporcionado
    usuario_query = session.query(Usuarios).filter(Usuarios.id == id)
...
async def eliminar_usuario(id:int):
    ...
    usuario = usuario_query.first()
    # Se confirman los cambios en la base de datos
    session.delete(usuario)    # se agregara todo 'con todas sus instancias'
    session.commit()        # commit es para que se ejecute la transaccion
...
@app.get ("/frutas/")
async def ver_todas_las_frutas():
    # Se obtienen todos los registros de la tabla Frutas
    frutas= session.query(Frutas).all()
    # Se verifica si hay al menos un registro en la tabla 'frutas'
    if not frutas:
        raise HTTPException(status_code= 404, detail="ninguna fruta encontrada")
    # si si hay value, retorna 'frutas'
...
@app.get("/frutas/{id}")
async def ver_fruta_por_id(id: int):
    # Se busca la fruta por el id proporcionado
    fruta_query = session.query(Frutas).filter(Frutas.id == id)
    # recibir el primer objeto que este en la tabla
...
@app.post("/frutas/{id}")
async def crear_fruta(id: int, nombre_fruta: str, cantidad_azucar: int):
    # Verificamos si ya existe una fruta con el mismo id
    existing_fruta = session.query(Frutas).filter(Frutas.id == id).first()
    ...
    # Agregamos la nueva fruta a la sesión usando session importado de 'db'
    session.add(nueva_fruta)
...
@app.put("/frutas/{id}")
async def actualizar_fruta_por_id(id: int, nombre_fruta: str, cantidad_azucar: int):
    # Verificamos si ya existe una fruta con el mismo id
    fruta_query = session.query(Frutas).filter(Frutas.id == id)
    # recibir el primer objeto que este en la tabla
    fruta = fruta_query.first()
...
async def asignar_id_propietario(id: int, propietario_id: int):
    # Verificamos si ya existe una fruta con el mismo id
    fruta_query = session.query(Frutas).filter(Frutas.id == id)
    # recibir el primer objeto que este en la tabla
    fruta = fruta_query.first()
...
async def eliminar_frutas(id: int):
    # Verificamos si ya existe una fruta con el mismo id
    fruta_query = session.query(Frutas).filter(Frutas.id == id)
    # recibir el primer objeto que este en la tabla
    fruta = fruta_query.first()
